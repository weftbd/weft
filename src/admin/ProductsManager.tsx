    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {editingProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-5 bg-slate-950/70 backdrop-blur-xs animate-fade-in">
          <div className="bg-white rounded-2xl max-w-2xl w-full border border-slate-200 shadow-2xl relative flex flex-col max-h-[92vh] sm:max-h-[90vh] overflow-hidden">
            {/* Pinned Sticky Header - Always visible on mobile & desktop */}
            <div className="flex items-center justify-between border-b border-slate-200 px-5 sm:px-7 py-4 bg-white shrink-0 z-10">
              <div>
                <h3 className="text-base sm:text-lg font-bold text-slate-900">
                  {isCreatingNew ? 'Create New Product' : `Edit Product: ${editingProduct.name}`}
                </h3>
                <p className="text-[11px] text-slate-500">
                  {isCreatingNew ? 'Add a new color/style to your store' : 'Update product details, pricing, and stock'}
                </p>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSaveProduct} className="flex flex-col grow overflow-hidden">
              <div className="p-5 sm:p-7 overflow-y-auto space-y-4 grow">
                {uploadError && (
                ...
                <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-200">
                  <div>
                    <span className="block text-xs font-semibold text-slate-700">Visible on Storefront</span>
                    <span className="block text-[11px] text-slate-500">
                      {editingProduct.active ? 'Product is active and shown to buyers' : 'Product is hidden from store'}
                    </span>
                  </div>
                  <button
                    type="button"
              ...
              {/* Pinned Sticky Footer - Always reachable */}
              <div className="flex items-center justify-end gap-3 px-5 sm:px-7 py-3.5 border-t border-slate-200 bg-slate-50 shrink-0">
                <button
                  type="button"
                  onClick={() => setEditingProduct(null)}
                  className="px-4 py-2 rounded-lg bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 text-xs font-semibold cursor-pointer transition-colors"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-5 py-2 rounded-lg bg-[#008236] hover:bg-[#006e2e] text-white text-xs font-semibold shadow-sm shadow-[#008236]/20 flex items-center gap-1.5 cursor-pointer transition-colors"
                >
