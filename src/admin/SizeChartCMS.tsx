...
import { Plus, Trash2, Save, CheckCircle2, TableProperties, Info, RefreshCw } from 'lucide-react';
import { SizeChart, SizeChartRow } from '../types';
import { saveSizeChart } from '../services/settings';
import { DEFAULT_SIZE_CHART } from '../data/defaults';
...
export const SizeChartCMS: React.FC<SizeChartCMSProps> = ({ sizeChart, onUpdated }) => {
  const [data, setData] = useState<SizeChart>(JSON.parse(JSON.stringify(sizeChart || DEFAULT_SIZE_CHART)));
  const [saving, setSaving] = useState(false);
  const [saveMessage, setSaveMessage] = useState<string | null>(null);

  // Sync with parent state when updated
  useEffect(() => {
    if (sizeChart) {
      setData(JSON.parse(JSON.stringify(sizeChart)));
    }
  }, [sizeChart]);
  ...
  const handleRowSizeChange = (rowIndex: number, newSize: string) => {
    ...
    rows[rowIndex] = { ...rows[rowIndex], size: newSize };
    setData({ ...data, rows });
  };

  const handleValueChange = (rowIndex: number, colIndex: number, val: string) => {
    const rows = [...data.rows];
    const newValues = [...rows[rowIndex].values];
    newValues[colIndex] = val;
    rows[rowIndex] = { ...rows[rowIndex], values: newValues };
    setData({ ...data, rows });
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSaveMessage('Size chart updated & published successfully!');
      onUpdated();
    } catch (e: any) {
  ...
  const handleResetDefaults = () => {
    setData(JSON.parse(JSON.stringify(DEFAULT_SIZE_CHART)));
    setSaveMessage('Reset to standard size specifications. Click "Save Size Chart" to publish.');
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3.5 py-2.5 rounded-lg border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 text-xs font-semibold shadow-xs transition-all cursor-pointer"
          >
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-semibold text-slate-700 flex items-center gap-2">
              ...
              <span>Measurements Matrix (Inches)</span>
            </span>
            <button
              type="button"
