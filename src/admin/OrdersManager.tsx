...
export const OrdersManager: React.FC<OrdersManagerProps> = ({
  ...
  }, [orders, statusFilter, dateFilter, startDate, endDate, searchQuery]);
  ...
  const handleConfirmSingleDelete = async () => {
    ...
    try {
      ...
      if (inspectOrder?.id === orderToDelete.id) setInspectOrder(null);
    } catch (e) {
  ...
  const statusList: { label: string; value: string; count: number }[] = [
    ...
    { label: 'Pending', value: 'PENDING', count: orders.filter((o) => o.orderStatus === 'PENDING').length },
    { label: 'Confirmed', value: 'CONFIRMED', count: orders.filter((o) => o.orderStatus === 'CONFIRMED').length },
    { label: 'Processing', value: 'PROCESSING', count: orders.filter((o) => o.orderStatus === 'PROCESSING').length },
    { label: 'Shipped', value: 'SHIPPED', count: orders.filter((o) => o.orderStatus === 'SHIPPED').length },
    ...
    { label: 'Returned', value: 'RETURNED', count: orders.filter((o) => o.orderStatus === 'RETURNED').length },
  ];
