...
export const StoreSettingsCMS: React.FC<StoreSettingsCMSProps> = ({
  ...
  const handleResetDefaults = () => {
    ...
    setSaveMessage({
      type: 'success',
      text: 'Default settings loaded. Click "Save Configuration" to persist.',
    });
  };
  ...
  return (
    <form onSubmit={handleSave} className="space-y-6 animate-fade-in max-w-4xl pb-12">
      ...
      {/* SECTION 5: Meta Pixel & Conversions API (CAPI) */}
      <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <div className="flex items-center gap-2">
            <Activity className="w-5 h-5 text-[#008236]" />
            <div>
              <h3 className="text-base font-bold text-slate-900">Meta Pixel & Conversions API (CAPI)</h3>
              <p className="text-xs text-slate-500">
                Track full e-commerce funnels with browser-to-server event deduplication
              </p>
            </div>
          </div>
          <label className="relative inline-flex items-center cursor-pointer">
            <input
              type="checkbox"
              checked={data.metaTrackingEnabled ?? true}
              onChange={(e) => setData({ ...data, metaTrackingEnabled: e.target.checked })}
              className="sr-only peer"
            />
            <div className="w-11 h-6 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-[#008236]"></div>
            <span className="ml-2 text-xs font-semibold text-slate-700">
              {data.metaTrackingEnabled !== false ? 'Enabled' : 'Disabled'}
            </span>
          </label>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
          <div>
            ...
            <input
              ...
              placeholder="1005176032544398"
              className="w-full px-3.5 py-2.5 rounded-lg border border-slate-200 bg-slate-50/50 text-slate-900 text-xs font-medium focus:bg-white focus:border-[#008236] focus:outline-none"
            />
            <p className="text-[11px] text-slate-400 mt-1">
              Active Pixel ID loaded in the customer storefront.
            </p>
          </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setData({ ...data, metaTestEventCode: e.target.value.trim() })}
              placeholder="e.g. TEST12345"
              className="w-full px-3.5 py-2.5 rounded-lg border border-slate-200 bg-slate-50/50 text-slate-900 text-xs font-medium focus:bg-white focus:border-[#008236] focus:outline-none"
            />
            <p className="text-[11px] text-slate-400 mt-1">
              Use for verifying server events live in Meta Events Manager test window.
            </p>
          </div>
        </div>

        <div className="mt-3 p-3.5 rounded-lg bg-emerald-50/60 border border-emerald-200/60 flex items-start gap-2.5">
          <ShieldCheck className="w-4 h-4 text-[#008236] shrink-0 mt-0.5" />
          <div className="text-[11px] text-emerald-900 space-y-1">
            <p className="font-semibold">Automated Server Deduplication & Privacy Hashing Active</p>
            <p className="text-slate-600">
              Orders trigger synchronized browser Pixel + server CAPI events sharing matching <code>event_id</code> (<code>purchase_WEFT-YYYY-XXXX</code>). Customer phone numbers and names are normalized and SHA256-hashed before transmission.
            </p>
          </div>
        </div>
      </div>
