...
export const OrderFormSection: React.FC<OrderFormSectionProps> = ({
  ...
  // Trigger InitiateCheckout when user interacts with the order form
  const handleInitiateCheckoutEvent = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedItems.length === 0) {
      ...
      setFormError('অনুগ্রহ করে পছন্দের অন্তত একটি শার্ট সিলেক্ট করুন।');
    ...
    // 1b. Validation: User MUST explicitly select a size for every selected product
    const itemsWithoutSize = selectedItems.filter((it) => !it.size || it.size.trim() === '');
    if (itemsWithoutSize.length > 0) {
      const missingProdIds = itemsWithoutSize.map((it) => it.productId);
      const missingProd = products.find((p) => p.id === itemsWithoutSize[0].productId);
      setFieldErrors({
        products: true,
        sizeErrorProductIds: missingProdIds,
      });
      setFormError(
        missingProd
          ? `অনুগ্রহ করে "${missingProd.name}" এর সাইজ (S, M, L, XL, XXL) সিলেক্ট করুন।`
          : 'অনুগ্রহ করে প্রতিটি সিলেক্টেড শার্টের সাইজ নির্বাচন করুন।'
      );
      if (productSectionRef.current) {
        productSectionRef.current.scrollIntoView({ behavior: 'smooth', block: 'center' });
      }
    ...
    // Track AddPaymentInfo (Cash on delivery)
    trackAddPaymentInfo({
      items: selectedItems.map((it) => {
        const prod = products.find((p) => p.id === it.productId);
        return {
    ...
    try {
      ...
      if (result.success && result.order) {
        // Track browser Meta Pixel Purchase event with matching deduplication eventId
        trackPurchase(result.order, (result as any).eventId);
  ...
  return (
    <section id="order-form" className="py-8 sm:py-16 bg-white">
      <div className="max-w-6xl mx-auto px-3 sm:px-6">
        ...
        <form onSubmit={handleSubmit} onFocusCapture={handleInitiateCheckoutEvent}>
            ...
            >
              ...
              <div className="space-y-3">
                {activeProducts.length === 0 ? (
                  ...
                  const isMissingSize = isSelected && !currentSize && (fieldErrors.sizeErrorProductIds?.includes(product.id) || !!fieldErrors.products);
                  ...
                  return (
                    <div
                      ...
                      className={`p-3 sm:p-4 rounded-xl border transition-all ${
                        isSelected
                          ? isMissingSize
                            ? 'border-rose-400 bg-rose-50/20 shadow-xs ring-1.5 ring-rose-400'
                    ...
                    >
                      ...
                      {isSelected && (
                        <div
                          ...
                          className="mt-3 pt-2.5 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2.5"
                        >
                          {/* Sizes Picker with explicit validation and red indicator */}
                          <div className="grow">
                            <div className="flex items-center gap-1.5 mb-1.5 flex-wrap">
                              <span className="text-[10px] sm:text-[11px] font-bold text-slate-700 uppercase tracking-wider">
                                সাইজ নির্বাচন:
                              </span>
                              {currentSize ? (
                                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[#008236]/15 text-[#008236] text-[10px] sm:text-[11px] font-bold">
                                  <Check className="w-3 h-3 stroke-[3]" /> {currentSize} সাইজ নির্বাচিত
                                </span>
                              ) : (
                                <span
                                  className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] sm:text-[11px] font-bold transition-all ${
                                    isMissingSize
                                      ? 'bg-rose-600 text-white shadow-xs animate-bounce'
                                      : 'bg-rose-50 text-rose-600 border border-rose-200'
                                  }`}
                                >
                                  <AlertCircle className="w-3 h-3" /> সাইজ বেছে নিন * (আবশ্যক)
                                </span>
                              )}
                            </div>

                            <div
                              className={`flex items-center gap-1.5 flex-wrap p-1 rounded-lg transition-all ${
                                isMissingSize
                                  ? 'bg-rose-50/80 ring-2 ring-rose-400 border border-rose-300 animate-shake'
                                  : ''
                              }`}
                            >
                                ...
                                return (
                                  <button
                                    ...
                                    onClick={(e) => {
                                      ...
                                      if (fieldErrors.sizeErrorProductIds?.includes(product.id)) {
                                        setFieldErrors((prev) => ({
                                          ...prev,
                                          sizeErrorProductIds: prev.sizeErrorProductIds?.filter(
                                            (id) => id !== product.id
                                          ),
                                        }));
                                      }
                                    }}
                                    className={`min-w-10 h-8 sm:min-w-9 sm:h-7.5 px-2.5 sm:px-2 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                                      isChosen
                                        ? 'bg-[#008236] text-white shadow-xs scale-105 ring-2 ring-[#008236]/30'
                                        : isOut
                                        ? 'bg-slate-100 text-slate-300 line-through cursor-not-allowed'
                                        : !currentSize
                                        ? 'bg-white hover:bg-emerald-50 text-slate-800 border-2 border-dashed border-slate-300 hover:border-[#008236] active:scale-95 shadow-2xs'
              ...
              <div className="space-y-3.5">
                ...
                <div>
                  ...
                  <input
                    ...
                    required
                    value={name || ''}
                    onFocus={handleInitiateCheckoutEvent}
                    onChange={(e) => {
