...
import { trackAddToCart, trackRemoveFromCart, trackViewContent } from '../../services/metaPixel';
...
export const StyleSelection: React.FC<StyleSelectionProps> = ({
  ...
  // Track ViewContent for first active product on mount
  useEffect(() => {
    if (activeProducts.length > 0) {
      trackViewContent({ product: activeProducts[0] });
    }
  }, [activeProducts.length]);

  const handleProductClick = (productId: string) => {
    const product = activeProducts.find((p) => p.id === productId);
    const isCurrentlySelected = selectedItems.some((item) => item.productId === productId);
    ...
    if (product) {
      ...
      if (isCurrentlySelected) {
        ...
        trackAddToCart({ product, quantity: 1, size: 'L' });
      }
    }
      ...
      {activeProducts.length === 0 ? (
        <div className="text-center py-12 px-4 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
          <p className="text-sm sm:text-base font-semibold text-slate-700">
            বর্তমানে কোনো প্রোডাক্ট ক্যাটালগে নেই
          </p>
          <p className="text-xs sm:text-sm text-slate-500 mt-1">
            অ্যাডমিন প্যানেল থেকে নতুন প্রোডাক্ট যুক্ত করুন
          </p>
        </div>
      ) : (
                  ...
                  >
                    {isSelected ? (
                      <>
                        ...
                        <span className="truncate">সিলেক্টেড</span>
                      </>
                    ) : (
                      <>
